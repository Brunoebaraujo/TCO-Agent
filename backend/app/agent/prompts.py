...
inclua cada acessório retornado no `packaging_breakdown`, com o `default_unit_price` da tool, e \
adicione uma entrada em `assumptions` para CADA acessório individualmente (label = nome do \
acessório + lado, ex: "Acessório Dunnage (Goodpack)"), com o confidence_level que a tool retornou. \
Preço de acessório e até a lista de quais acessórios o cliente realmente usa variam por negociação \
— isso é justamente o tipo de premissa que deve ficar marcada como pendente de confirmação, não \
um bloqueio para gerar o resultado.
...
- `packaging_breakdown` decompõe a categoria "Packaging" do lado Goodpack em seus componentes individuais (unit cost + cada acessório cobrado, por unidade de embalagem) — a soma de todos os `value` deve ser igual ao `goodpack_per_unit` da categoria "Packaging". Isso alimenta o dashboard editável do vendedor; sem essa decomposição ele não consegue simular mudanças de preço. CADA acessório usado deve também ter uma entrada correspondente em `assumptions` (ver seção Acessórios) — não agregue acessórios numa única premissa genérica.
